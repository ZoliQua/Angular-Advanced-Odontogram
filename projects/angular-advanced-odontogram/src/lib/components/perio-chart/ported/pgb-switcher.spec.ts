// Angular port of core/__tests__/pgb-switcher.test.ts.
//
// The source's `drawArchOverlay` import from `../PerioChart` maps to this
// repo's `perio-grid-dom.ts`; the pure geometry/DOM builders
// (`buildBuccalArchSvg`/`buildPalatalArchSvg`/`perioOverlayMarks`/
// `perioPlaqueMarks`/`buildPerioOverlayLayer`) map to core `perioGraphic.ts`
// — see pgb-mm-overlays.spec.ts's header for the same mapping rationale.
// The switcher UI describe mounts PerioChartComponent inline.
//
// .trim() on switch-button textContent: the Angular template's
...
describe("PG-B Task 2: #perioOverlaySwitch", () => {
  it("renders a radio-style switch row with a button per implemented layer", async () => {
    const f = openInline();
    await f.whenStable();
  ...
  it("None is active by default", async () => {
    const f = openInline();
    await f.whenStable();
  ...
  it("clicking BOP selects it (getPerioOverlayLayer + active state) and shows %BOP", async () => {
  ...
  it("clicking Plakk selects it and shows PI%", async () => {
    setPlaque(16, "buccal", true);
    const f = openInline();
    await f.whenStable();
  ...
  it("selecting None clears the active selection + read-out", async () => {
    const f = openInline();
    await f.whenStable();
    const bopBtn = document.querySelector('[data-overlay-layer="bop"]') as HTMLButtonElement;
    bopBtn.click();
