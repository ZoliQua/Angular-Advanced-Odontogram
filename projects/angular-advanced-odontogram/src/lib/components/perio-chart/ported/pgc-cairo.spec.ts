// Angular port of core/__tests__/pgc-cairo.test.ts.
//
// Same mapping as pgb-switcher.spec.ts/pgb-mm-overlays.spec.ts:
// `drawArchOverlay` (source: `../PerioChart`) -> this repo's
// `perio-grid-dom.ts`; `buildBuccalArchSvg`/`buildPalatalArchSvg`/
// `perioCairoMarks` (source: `../perioGraphic`) -> core `perioGraphic.ts`.
// The switcher UI describe mounts PerioChartComponent inline; `.trim()` on
// its textContent assertion for the same reason as those two files.
...
describe("PG-C Task 1: #perioOverlaySwitch offers Cairo", () => {
  it("renders a Cairo button labeled via perio.overlay.cairo", async () => {
    const f = openInline();
    await f.whenStable();
    const sw = document.getElementById("perioOverlaySwitch")!;
    const btn = sw.querySelector('[data-overlay-layer="cairo"]');
    expect(btn).toBeTruthy();
    expect(btn!.textContent!.trim()).toBe(t("perio.overlay.cairo"));
  });

  it("clicking Cairo selects it (getPerioOverlayLayer + active state)", async () => {
    const f = openInline();
    await f.whenStable();
  ...
  it("selecting None after Cairo clears the active selection", async () => {
    const f = openInline();
    await f.whenStable();
    const cairoBtn = document.querySelector('[data-overlay-layer="cairo"]') as HTMLButtonElement;
    cairoBtn.click();
