// Angular port of core/__tests__/ui2-index-names.test.ts.
//
// PerioChart mount route mirrors perio-p2-grid.spec.ts (see its header for
// the full rationale). PerioSidebar mounts directly via
// `TestBed.createComponent(PerioSidebarComponent)`, the same route
// perio-sidebar/ported/ui1-perio-sidebar.spec.ts uses.
//
// .trim() on overlay-switch pill textContent: the Angular template's
// `@for`/interpolation whitespace formatting leaves surrounding text nodes
// the React JSX didn't have; substance (the resolved label) is unchanged —
// see pgb-mm-overlays.spec.ts's header for the same fix.
...
function overlayPillLabels(): Record<string, string> {
  ...
  for (const btn of Array.from(buttons)) {
    out[btn.dataset["overlayLayer"]!] = (btn.textContent ?? "").trim();
  }
  return out;
}
...
beforeEach(() => {
  ...
  // (see ui3b-mpi-implant-gate.spec.ts) — set one so the rows render across
...
describe("UI-2 Task 3: PerioChart grid row labels", () => {
  it("translated mode (default) renders the existing t(...) labels", async () => {
    const f = openGrid();
    await f.whenStable();
  ...
  it("canonical mode renders the fixed canonical strings, including where they differ from en", async () => {
    setPerioIndexNameMode("canonical");
    const f = openGrid();
    await f.whenStable();
    const labels = rowLabels();
  ...
  it("rebuilds the grid live when the mode flips after mount", async () => {
    const f = openGrid();
    await f.whenStable();
  ...
  it("the PD/GM/CAL/BOP composed row labels are unaffected (already a fixed abbreviation)", async () => {
    const f = openGrid();
    await f.whenStable();
...
describe("UI-2 Task 3: info-tooltip text is ALWAYS translated", () => {
  it("stays t('perio.info.pi') in translated mode", async () => {
    const f = openGrid();
    await f.whenStable();
    expect(infoPopoverTextFor("Plaque Index (PI)")).toMatch(/Silness/i);
  });

  it("stays t('perio.info.pi') in canonical mode too (tooltip never canonicalizes)", async () => {
    setPerioIndexNameMode("canonical");
    const f = openGrid();
    await f.whenStable();
    expect(infoPopoverTextFor("Plaque Index (PI)")).toMatch(/Silness/i);
  ...
  it("stays t('perio.info.mbi') in canonical mode even though the row label itself changed", async () => {
    setPerioIndexNameMode("canonical");
    const f = openGrid();
    await f.whenStable();
...
describe("UI-2 Task 3: overlay switcher pill labels", () => {
  it("translated mode shows the existing t('perio.overlay.<layer>') text", async () => {
    const f = openGrid();
    await f.whenStable();
  ...
  it("canonical mode routes matching layers through indexName(), unaffected layers stay translated", async () => {
    setPerioIndexNameMode("canonical");
    const f = openGrid();
    await f.whenStable();
    const pills = overlayPillLabels();
...
describe("UI-2 Task 3: PerioSidebar whole-mouth summary labels", () => {
  ...
  it("translated mode (default) renders the existing composed summary labels", async () => {
  ...
  it("canonical mode renders the fixed English/Latin composed summary labels", async () => {
    setPerioIndexNameMode("canonical");
    const f = renderSidebar();
    await f.whenStable();
    expect(summaryLabelText("perio-fg-summary-avgpd")).toBe("Avg PD");
  ...
  it("re-renders live when the mode flips after mount (onStateChange)", async () => {
    const f = renderSidebar();
    await f.whenStable();
    expect(summaryLabelText("perio-fg-summary-maxfurc")).toBe("Max furcation");
    setPerioIndexNameMode("canonical");
