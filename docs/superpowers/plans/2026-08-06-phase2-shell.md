...
> **Correction (post-Task-4 review, controller-ruled):** the "React conditional visibility for `showStatusCard`/`showOrthoCard` → `@if`" instruction above was wrong. App.tsx 782/891 keep `#statusCard`/`#orthoCard` permanently MOUNTED — `className={showStatusCard ? "" : "hidden"}` on a wrapper `<div>`, never a conditional that unmounts the section. The engine dereferences `#btnResetAll`, `#btnPrimaryDentition`, `#btnMixedDentition`, `#orthoApplianceSelect`, `#orthoRotationToggle`, and `#orthoCard` without null guards during `initOdontogram()`/sync (odontogram.ts:8692/8711/8648/8657/3899), so unmounting them via `@if` breaks boot whenever a host passes `false`. Correct mapping: `<div [class.hidden]="!showStatusCardOn()">`/`<div [class.hidden]="!showOrthoCardOn()">` wrapping the always-mounted `#statusCard`/`#orthoCard` sections, transcribing the wrapper exactly as App.tsx's `className={showX ? "" : "hidden"}` shape. The same "stays mounted, CSS-hidden" rule applies to the `.chart-column` (App.tsx 659-662, viewMode-dependent `style`) and `.panel-odontogram-controls` (App.tsx 746, isPerioView-dependent `style`) wrapper divs — both restored in the Task 4 fix pass, their dynamic `style` bindings deferred to Task 5.
