...
const REACT_DEPENDENT = [
  ...
  "case-conditions-ui.test.tsx", // covered-by: components/case-diagnoses-modal/case-diagnoses-modal.component.spec.ts (Task 3's 3 real-engine tests + Task 5's 3 gap-close tests, 6 total: dialog open/close with no engine reads while closed, add-via-picker write-through + force-value reset + delete, a lateralizable row's laterality select write-through, a non-lateralizable row hides the laterality select, the add-select is code-first/code-sorted, and it excludes catalog keys already active). Task 5 assertion-inventory diff against the pinned 10-test upstream file found these 3 real gaps in Task 3's version and closed them (real engine, not upstream's mocked getCaseConditions/setCaseCondition — the "renders no rows/renders nothing when open=false" cases are the real engine's own zero-state, not separately asserted).
  ...
  "dx-card.test.tsx", // covered-by: components/surfaces/cards/diagnoses-card.component.spec.ts (Task 3's 5 real-engine tests + Task 5's 2 gap-close tests, 7 total: derived-row code-first render + exclude toggle + delete, section visibility gate, exclude write-through, delete clears the chart axis, add-picker write-through + force-value reset, an ADDED row's 'added' tag/no-exclude-toggle/delete, toggling suppress back off). Task 5 assertion-inventory diff against the pinned 11-test upstream file (which mocks getActiveDiagnoses/setDxOverrideForSelection/addDiagnosisToSelection/removeDiagnosisFromSelection to a canned view-model) found the "added"-source row path untested — closed via setDxOverrideForSelection(key,"add") directly (the #dxAddSelect picker itself always writes a REAL chart axis, so it can only ever produce a "derived" row; an "added" row needs a dxOverride with no matching raw finding, same reason upstream's own test has to mock the view-model to exercise it at all). NOT ported: the "no code" edge-case rows and the trailing i18n-key-presence describe block (reads the real translations table across all 12 languages directly, independent of the render mocks) — the latter is a content-completeness check with no Angular-specific behavior to port; every key it checks already exists in the resynced locales/*.ts per Task 1's own diff-discipline audit.
  ...
  "get-selected-teeth.test.ts", // ported: components/odontogram-shell/ported/get-selected-teeth.spec.ts (4 framework-free cases) + public-api-surface.spec.ts (the 5th case's "is part of the public entry point" assertion, ported against public-api.ts instead of React's ../App — see get-selected-teeth.spec.ts's own header for why)
  "i18n-lazy-load.test.tsx", // covered-by: lib/lazy-loading.guard.spec.ts (Task 2; test:ng, not test:corpus — the file's own earlier annotation misstated the runner). The 2 framework-free static-analysis guards ("no static import of a non-English locale/translations.ts"; "the loader reaches every language via a literal dynamic import") are ported 1:1, additionally scoped to ALL of src/ not just lib/core (see task-2-report.md §3/§5, which is how this task's own public-api.ts translations-inlining regression was caught). The 5 remaining i18n-focused tests in that spec (2 more of its 9 are the SEPARATE measured-anatomy guard) prove the ANGULAR CONSUMPTION layer doesn't defeat the core bus's own async-switch/first-paint-gate guarantees (bus-level "stays on previous until it arrives", the Angular languageReady gate open/closed, real language-menu wiring, rapid-switch settling) — the bus mechanics themselves (retry-after-failed-load, concurrent-request dedup, useI18n's no-flash-of-fallback) are core-verbatim/untouched by this port and stay covered only by the frozen upstream test, not independently re-verified here (residual gap, same class as this file's siblings below).
  ...
  "parity/api-surface.test.ts", // covered-by: lib/public-api-surface.spec.ts (Task 5). Upstream freezes EVERY runtime export of React's `../../App`/`../../odontogram` as a byte-identical golden (>300/>100 entries) — a safety net for upstream's OWN odontogram.ts split. A byte-identical golden has no useful Angular analog (our public-api.ts is intentionally a smaller, curated surface, not odontogram.ts's raw export list); the ported spec instead asserts every NEW v2.6.0 dx/case-condition symbol and the 2 new UI components actually reach public-api.ts, a `translations`-record regression guard (see public-api.ts's own header comment / task-2-report.md §5 — the exact class of silent-breakage bug this upstream golden exists to catch), and a sanity check that the surface stays large with no undefined entries.
