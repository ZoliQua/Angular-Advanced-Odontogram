...
const REACT_DEPENDENT = [
  ...
  "perio-chart-anatomy-switch.test.tsx", // phase-11: mounts <PerioChart/> to pin the "resetPerioTemplateCache() depends on the anatomy profile" bug fix (measured teeth silently vanishing from an open perio chart after a mid-session anatomy switch). Port target: Task 4 (perio deltas).
  ...
  "selection-controls-enable.test.tsx", // phase-11: mounts <App/> to pin the selection-change control-panel-enable performance fix (one-pass label gathering, replacing a per-control `document.querySelector` over the whole tooth grid). Port target: Task 4 (shell/surfaces).
